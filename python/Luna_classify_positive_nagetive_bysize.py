...
y_train=np.round(y_train/1).astype('uint8')
y_test=np.round(y_test/1).astype('uint8')
#y_train=np.array(y_train>0,'uint8')
#y_test=np.array(y_test>0,'uint8')
...
for epoch in range(params_train['nbepoch']):
    ...
    score_test=model.evaluate(np.array(X_test)[:,np.newaxis],y_test,verbose=0,batch_size=bs)
