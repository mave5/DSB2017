...
# save dsb nodes
path2dsb_nodes=weightfolder+'/dsb_nodes.hdf5'
if not os.path.exists(path2dsb_nodes):
    ff_yp=h5py.File(path2dsb_nodes,'w-')
    for k3,id2 in enumerate(df_train.id):
        print k3,id2
        X_batch,y_batch=extract_dsb([id2])    
        ff_yp[id2]=X_batch
        ff_yp[id2].attrs['cancer']=y_batch[0]
    ff_yp.close()    
else:
    ff_yp=h5py.File(path2dsb_nodes,'r')
        
    
    print 'dsb nodes exist'
...
for epoch in range(params_train['nbepoch']):
    ...
    bs2=8*bs
    for t1 in range(0,len(trn_ids),bs2):
        trn_id_batch=trn_ids[t1:t1+bs2]                
        print t1
    
        # extract nodule masks 
        #X_batch,y_batch=extract_dsb(trn_id_batch)    
        X_batch=np.zeros((0,timestep,1,h,w),'float32')
        y_batch=[]
        for id3 in trn_id_batch:
            X_batch0=np.array(ff_yp[id3])
            X_batch0=X_batch0[np.newaxis,:]
            X_batch0,_=iterate_minibatches(X_batch0,X_batch0,X_batch0.shape[0],shuffle=False,augment=True)        
            X_batch0=X_batch0[:,:,np.newaxis]
            X_batch=np.append(X_batch,X_batch0,axis=0)            
            y_batch.append(ff_yp[id3].attrs['cancer'])
            
        # fit model to data
        model_rnn.fit(X_batch, np.array(y_batch), nb_epoch=1, batch_size=bs,verbose=0,shuffle=True)    

    # evaluate on test and train data
    X_test=np.zeros((0,timestep,1,h,w),'float32')
    y_test=[]
    for id4 in val_ids:
        X_t0=np.array(ff_yp[id4])
        X_t0=X_t0[np.newaxis,:,np.newaxis]
        X_test=np.append(X_test,X_t0,axis=0)
        y_test.append(ff_yp[id4].attrs['cancer'])
