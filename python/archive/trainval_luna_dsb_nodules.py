...
experiment='classify_nodules_bs4'+'_hw_'+str(h)+'by'+str(w)
...
for epoch in range(params_train['nbepoch']):
    ...
    X_batch,y_batch=extract_dsb_seq(trn_nc,bs/8,df_train)        
    ...
    X,y=extract_dsb_seq(val_nc,bs/8,df_train)        
...
for k in range(len(y_pred)):
    yp_avg.append(np.mean(y_pred_nc[k]))
    yp_max.append(np.max(y_pred_nc[k]))
    n=3
    yk=np.array(y_pred_nc[k])
    idx = (-yk).argsort()[:n]
...
for k in range(len(y_pred_cancer)):
    ...
    y_p_cancer_sum.append( np.sum(y_pred_cancer[k]))

plt.hist(y_p_cancer_sum)
plt.hist(yp_cancer_max,100)
# save cancer results
