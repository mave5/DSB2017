...
def collect_features(data_frame,dsb_type='train'):
    ...
    for k,id in enumerate(data_frame.id):
        ...
        if len(coords)>0:
            ...
            for ns in range(1,5):
                xs=coords[coords[:,2]>ns*4,0]
                ys=coords[coords[:,2]>ns*4,1]
                zs=z_locs[coords[:,2]>ns*4]
            ...
            fig.savefig('./output/features/nodefigs/'+id+'.jpg')
            ...
            for k1 in range(num_regs):
                if len(coords[labels==k1])>0:
                    ...
                    tmp5=np.median(coords[labels==k1,2])                                             
...
for i, (train, test) in enumerate(skf[2:]):
            ...
            x_test = X_dsb[test]
            y_test = y_dsb[test]
            clf.fit(x_train, y_train, eval_set=[(x_test, y_test)], verbose=True, eval_metric='logloss', early_stopping_rounds=50)
