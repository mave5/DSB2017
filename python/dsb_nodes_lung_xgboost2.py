...
if not os.path.exists(path2dsb_features): 
    np.savez(path2dsb_features,X_dsb=X_dsb,X_dsb_test=X_dsb_test,y_dsb=y_cancer)
...
for i, (train, test) in enumerate(skf):
            ...
            x_test = X_dsb[test]
            y_test = y_dsb[test]
            clf1.fit(x_train, y_train, eval_set=[(x_test, y_test)], verbose=False, eval_metric='logloss', early_stopping_rounds=20)
            y_pred=clf1.predict(x_test,ntree_limit=clf1.best_iteration)
            loss.append(log_loss(y_test,y_pred))
            y_dsb_pred+=clf1.predict(X_dsb_test)
